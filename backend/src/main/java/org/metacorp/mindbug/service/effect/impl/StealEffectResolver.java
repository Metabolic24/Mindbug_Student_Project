...
public class StealEffectResolver extends EffectResolver<StealEffect> {
    ...
    public void apply(Game game, CardInstance card, EffectTiming timing) {
        ...
        List<CardInstance> availableCards = switch (source) {
            case DISCARD -> opponents.stream().flatMap(opponent -> opponent.getDiscardPile().stream()).collect(Collectors.toList());
            case HAND -> opponents.stream().flatMap(opponent -> opponent.getHand().stream()).collect(Collectors.toList());
            case SELF_DISCARD -> cardOwner.getDiscardPile();
            case BOARD -> opponents.stream().flatMap(opponent -> opponent.getBoard().stream()).collect(Collectors.toList());
        };
